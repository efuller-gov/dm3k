...
function generateSolnMatrix(data, outline, widthFunc){
    // Plotting vars
    ...
    for (let j=0; j<outline.activityInstances.length; j++){
        ...
        instance_table = u_activity_instance.instanceTable;
        for (let i=0; i<instance_table.length; i++){
            cur_act_inst = instance_table[i]
            ...
            cur_class = instance_table[i].instanceName.split("_")[0].toLowerCase();            
            let agg_reward = inheritContainerRewards(
                cur_class, 
    ...
    for (let a of act_list) {
        reward = 0;
        cur_class = a.split("_")[0].toLowerCase();
    ...
    x_pad = _.max([get_pad(activity_dict), get_pad(resource_dict)])
    y_pad = _.max([get_pad(activity_dict), get_pad(resource_dict)])
    class_padding = get_class_pad(x_pad)
    classBoxSize = get_class_box_size(x_pad)
    // x_pad = (activity_dict.length>25) ? 5 : 10;
    // y_pad = (resource_dict.length>25) ? 5 : 10;

    containerWidth = svgContainerWidth - res_containers.length*classBoxSize - classBoxSize*2 - activity_dict.length*x_pad - 
        (instances_per_activity_class.length)*class_padding;
    containerHeight = containerWidth/1.4 -  act_containers.length*classBoxSize*2 - (instances_per_resource_class.length-1)*class_padding
    ...
    for (let rc of res_containers){
        ri = resource_dict.filter(x=>x.class==rc.childClassLabel)
...
function drawSolnOutput(
    ...
    let activityAx = grid.selectAll(".activityAx")
        .data(instances_per_activity_class)
            .enter().append("g")
            .attr("transform", function(d, i) {
                xt = classBoxSize*3 + x_ax_pad;
                yt = _.max(table.map(x=>x.y)) + table.filter(x=>x.y==_.max(table.map(x=>x.y)))[0].h + 
    ...
    for (let i=0; i<act_containers.length; i++){
        ...
        for (let ac_name of _.uniq(ac.instanceTable.map(x=>x.parentInstanceName))){
            ...
            aci = ac.instanceTable.filter(x=>x.parentInstanceName==ac_name)
...
function inheritContainerRewards(classLabel, instanceName, containerList, outlineInstances, cumReward){
    ...
    if (containerClass!= undefined){
        ...
        for (let i=0; i<containinedInstances.length; i++){
            ...
            cumReward = inheritContainerRewards(containerClass.parentClassLabel, parentInstanceName, containerList, outlineInstances, cumReward)
        }
    } else{
        return cumReward
    }
...
function getChild(act_containers, childClassLabel){
...
function get_pad(instance_dict){
    let x_pad_dict = [
        {
            num_inst: 0,
            pad: 8,
...
function get_class_box_size(x_pad){
    let b = get_class_pad(x_pad)
    return b
}